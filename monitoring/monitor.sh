...
while(($step<$total_loop));
	do
		...
		timeout -s 1 ${time_window} perf trace -o ${folder}/${path}.txt -e !nanosleep -T -p ${pid};
		python preprocessing_pref.py  ${folder}/${path}.txt ${folder}/${path}.csv
		rm -rf ${folder}/${path}.txt
